...
def train(config, model, train_dataset, dev_dataset, loss_function):
    ...
    for epoch in range(config.num_epochs):
        ...
        for inputs in data:
            ...
            if tempnum % 200 == 0 and tempnum != 0:
                (precision, recall, macro_f1, _), dev_loss, micro_f1 = evaluate(config, model, dev_dataset,
                                                                                loss_function=loss_function)
                writer.add_scalar("loss/train", total_loss / len(train_dataset), tempnum)
                # writer.add_scalar("loss/flood", flood_loss, epoch)
                writer.add_scalar("loss/dev", dev_loss / len(dev_dataset), tempnum)
                writer.add_scalars("performance/f1", {'macro_f1': macro_f1, 'micro_f1': micro_f1}, tempnum)
                writer.add_scalar("performance/precision", precision, tempnum)
                writer.add_scalar("performance/recall", recall, tempnum)
...
if __name__ == '__main__':
    config = Config(dataset='../../dataset', name='Bert4layerCNN-PGD-base9bert')
    ...
    # 因为一个迭代需要一个半小时，batchsize=8的时候需要1401个step，
    train(config, model, train_dataloader, dev_dataloader, loss_function=ASLSingleLabel())
